...
- Autonomous ARP/NS transmission is gated by LAN link state. Unresolved IPv6
  ...
  datapath lcores for a single rebuild attempt per affected port:
  `rte_eth_dev_reset()` (a plain stop on PMDs without the op), reconfigure,
  restart, and restore VLAN filters. Traffic resumes immediately afterwards,
  so a port whose PF is not ready yet logs "recovery degraded" and is retried
  by the next 500 ms supervisor tick instead of stretching the paused window.
  The control lcore then sends one gratuitous ARP and one unsolicited NA,
  each with its own "sent ... on LAN port" log line. No WAN warm-up is needed
  because PPPoE uses session IDs rather than neighbour discovery, and port
  rebuild already restores the VF's MAC filters.
